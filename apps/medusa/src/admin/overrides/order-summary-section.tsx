// @ts-nocheck — Dashboard fork; ~dashboard imports match vite-plugin-unlock (see medusa-admin docs).
...
import { ActionMenu } from "~dashboard/components/common/action-menu"
import { Thumbnail } from "~dashboard/components/common/thumbnail"
import { useClaims } from "~dashboard/hooks/api/claims"
import { useExchanges } from "~dashboard/hooks/api/exchanges"
import { useOrderPreview } from "~dashboard/hooks/api/orders"
import { useMarkPaymentCollectionAsPaid } from "~dashboard/hooks/api/payment-collections"
import { useReservationItems } from "~dashboard/hooks/api/reservations"
import { useReturns } from "~dashboard/hooks/api/returns"
import { useDate } from "~dashboard/hooks/use-date"
import { formatCurrency } from "~dashboard/lib/format-currency"
import {
  getLocaleAmount,
  getStylizedAmount,
  isAmountLessThenRoundingError,
} from "~dashboard/lib/money-amount-helpers"
import { getTotalCaptured } from "~dashboard/lib/payment"
import { getReturnableQuantity } from "~dashboard/lib/rma"
import { CopyPaymentLink } from "~dashboard/routes/orders/order-detail/components/copy-payment-link/copy-payment-link"
import ReturnInfoPopover from "~dashboard/routes/orders/order-detail/components/order-summary-section/return-info-popover"
import ShippingInfoPopover from "~dashboard/routes/orders/order-detail/components/order-summary-section/shipping-info-popover"

import { OrderStripePayoutBreakdown } from "../components/order-stripe-payout-breakdown"
...
const CostBreakdown = ({
  ...
  return (
    <div className="text-ui-fg-subtle flex flex-col gap-y-2 px-6 py-4">
      ...
      <OrderStripePayoutBreakdown order={order} />
    </div>
  )
}
