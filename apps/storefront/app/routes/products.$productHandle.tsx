...
import { fetchSushiProductByHandle } from '@libs/util/server/sushi-products.server';
...
export const loader = async (args: LoaderFunctionArgs) => {
  ...
  const handle = args.params.productHandle as string;

  let product =
    (await fetchSushiProductByHandle(handle)) ??
    (
      await fetchProducts(args.request, {
        handle,
        fields:
          '*categories,*images,thumbnail,variants.*,variants.sku,variants.options,variants.inventory_quantity,variants.manage_inventory',
      })
    ).products[0];
